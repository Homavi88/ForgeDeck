...
- Cue output selection is synchronized from the popup back to the studio, so `setSinkId` updates both audio elements. CC deck bindings are edge-triggered; insert drag lands consistently on its drop target.
